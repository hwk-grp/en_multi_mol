...
import torch
import utils.ml
import utils.chem_rand_an_hy as chem
...
model_path = '../../pretrained_gcn/gcn_an_hy_2024.pth' # from 2021 to 2030
...
problem = GNNProblem(gcn_model=model, id_target=id_target, list_atom_types=list_atom_types, elem_angles=elem_angles,
                     lb=lbs, ub=ubs, batch_size=batch_size, n_fp=n_fp, n_radius=n_radius)
...
df.to_excel(f'../../feats/aeo/2.an_hy/en_an_hy_gcn_aeo_2_{seed_train}.xlsx', index=False)
...
df.to_excel(f'./preds/preds_en_an_hy_gcn_aeo_2_{seed_train}.xlsx', index=False)
