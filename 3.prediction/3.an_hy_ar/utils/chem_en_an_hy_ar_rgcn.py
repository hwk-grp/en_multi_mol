...
def find_non_zero_bt(id_target, new_en):
    ...
    for molecule in tqdm(range(0, id_target.shape[0])):
        ...
        atom_aromatics = [1 * atom.GetIsAromatic() for atom in mol.GetAtoms()]
        for i in range(n_atoms):
            at1_i = (atom_nums[i], int(atom_aromatics[i]), atom_hybrids[i])
            at1_i = str(at1_i)
            for j in range(n_atoms):
                if adj_mat[i, j] == 1:
                    at1_j = (atom_nums[j], int(atom_aromatics[j]), atom_hybrids[j])
                    at1_j = str(at1_j)
...
def smiles_to_mol_graph(n_fp, n_radius, smiles, idx, target, new_en, n_rel, atom_type_list, en_bond_type,
    ...
    for i in range(0, n_atoms):
        ...
        atom_feat_i = str((atom_i.GetAtomicNum(), int(atom_i.GetIsAromatic()), atom_i.GetHybridization()))
        ...
        for j in range(i+1, n_atoms):
            if adj_mat[i, j] == 1:
                ...
                atom_feat_j = str((atom_j.GetAtomicNum(), int(atom_j.GetIsAromatic()), atom_j.GetHybridization()))
