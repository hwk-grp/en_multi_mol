...
def load_dataset(path_user_dataset, list_bond_types, n_fp, n_radius, task, path_opt_feat):
    list_mols = []
    id_target = np.array(pd.read_excel(path_user_dataset))
    ...
    for i in tqdm(range(0, id_target.shape[0])):
        mol = smiles_to_mol_graph(n_fp, n_radius, id_target[i, 0], idx=i, target=id_target[i, 1],
                                  list_bond_types=list_bond_types, task=task, new_en=new_en)
        if mol is not None:
...
def smiles_to_mol_graph(n_fp, n_radius, smiles, idx, target, list_bond_types, task, new_en):
    # Smiles to a RDKit object
    mol = Chem.AddHs(Chem.MolFromSmiles(smiles))
    # mol = Chem.MolFromSmiles(smiles)
    adj_mat = Chem.GetAdjacencyMatrix(mol)
