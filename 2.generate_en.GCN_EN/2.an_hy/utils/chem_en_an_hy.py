...
def load_dataset(path_user_dataset, list_atom_types, list_bond_types, n_fp, n_radius, task):
    list_mols = []
    id_target = np.array(pd.read_excel(path_user_dataset))
...
def smiles_to_mol_graph(n_fp, n_radius, smiles, idx, target, list_atom_types, list_bond_types, task):
    # smiles to a RDKit object
    mol = Chem.AddHs(Chem.MolFromSmiles(smiles))
    #mol = Chem.MolFromSmiles(smiles)
