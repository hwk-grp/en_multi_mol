...
import torch
import utils.ml
import utils.chem_rand_an as chem
...
class GNNProblem(mealpy.utils.problem.Problem):
    def __init__(self, gcn_model, id_target, elem_angles, lb, ub, batch_size, n_fp, n_radius, minmax='min', **kwargs):
        self.bounds = None
        ...
        self.pre_data = chem.load_dataset(self.id_target,  self.n_fp, self.n_radius, self.elem_angles)
...
model_path = '../../pretrained_gcn/gcn_an_2024.pth' # from 2021 to 2030
...
opt_epochs = 500
num_elem_types = len(chem.list_elem_types)

# Load dataset
print('Load molecular structures...')
random.seed(seed_feat)
elem_angles = np.random.uniform(0, 2*np.pi, size=num_elem_types)
...
lbs, ubs = [0] * num_elem_types, [2*np.pi]*num_elem_types
model = GCN(2, n_mol_feats, dims, n_layers)
model.load_state_dict(trained_gcn_state_dict)

problem = GNNProblem(gcn_model=model, id_target=id_target, elem_angles=elem_angles, lb=lbs, ub=ubs,
                     batch_size=batch_size, n_fp=n_fp, n_radius=n_radius)
...
result = []
elem_list_symbol = ['H', 'C', 'N', 'O', 'F']
for i in range(num_elem_types):
    angle = best_sol[i]
    cos, sin = np.cos(angle), np.sin(angle)
    result.append([elem_list_symbol[i], cos, sin])
df = pd.DataFrame(result)
df.columns = ['Elem type', 'cos', 'sin']
df.to_excel(f'../../feats/aeo/1.an/en_an_gcn_aeo_2_{seed_train}.xlsx', index=False)

# Generate training and test datasets
best_sol = best_sol.reshape(-1, 1)
train_test_id = id_target_all[n_opt:]
train_test_data = chem.load_dataset(train_test_id, n_fp, n_radius, best_sol)
...
df.to_excel(f'./preds/preds_en_an_gcn_aeo_2_{seed_train}.xlsx', index=False)
