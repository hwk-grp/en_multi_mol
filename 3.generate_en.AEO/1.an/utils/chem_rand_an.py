...
def get_elem_angles_rand(path_user_dataset, seed):
    np.random.seed(seed)
    list_atom_types = []
    elem_angles = np.random.uniform(0, 2*np.pi, size=len(list_elem_types))
    elem_angles = elem_angles.reshape(-1, 1)
    return list_atom_types, elem_angles


def load_dataset(id_target, n_fp, n_radius, elem_angle):
    list_mols = []
    for i in tqdm(range(0, id_target.shape[0])):
    #for i in range(0, id_target.shape[0]):
        mol = smiles_to_mol_graph(n_fp, n_radius, id_target[i, 0], idx=i, target=id_target[i, 1], elem_angle=elem_angle)
...
def smiles_to_mol_graph(n_fp, n_radius, smiles, idx, target, elem_angle):
    ...
    for i in range(0, n_atoms):
        atom_num = atom_nums[i]
        angle_i = elem_angle[list_elem_types.index(atom_num)]
        ...
        ndx_i = list_elem_types.index(atom_num)
